...
#[derive(Debug)]
pub(crate) struct GasOracleCache {
...
impl GasOracleCache {
    ...
    pub(crate) fn get(&self, current_block: u64) -> Option<U256> {
        let cached_block = self.block.load(Ordering::Acquire);
        (cached_block == current_block).then(|| U256::from(self.tip.load(Ordering::Acquire)))
    }

    /// Update cache for new block.
    pub(crate) fn set(&self, block: u64, tip: U256) {
